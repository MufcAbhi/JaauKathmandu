        ...
        <div class="col-lg-3" id="sideprofile">
            <div class="row-fluid"> 
                    ...
                    <div id="sideprofileimg">
                            <img class="img-circle" src="img/users/<?php echo $login_session_pic; ?>">
                    </div>
...
$connection = mysqli_connect('localhost','root','','majorproject');
    ...
    if(isset($_POST['userlocation']) && !empty($_POST['userlocation'])){
            ...
            $query1 = mysqli_query($connection,"SELECT * FROM tbl_domestic_flight WHERE DepatureLocation LIKE '%$depaturecheck%' AND Dates Like '%$datecheck%' AND ArrivalLocation Like '%$arrivalcheck%' and Dates>=curdate()") or die("Could not search!");
            
            $count1 = mysqli_num_rows($query1);
                    


            $query2=mysqli_query($connection,"SELECT * FROM tbl_domestic_flight WHERE ArrivalLocation Like '%$arrivalcheck%'");
            $count2=mysqli_num_rows($query2);
            
            $query3=mysqli_query($connection,"SELECT * FROM tbl_domestic_flight WHERE Dates Like '%$datecheck%' AND Dates>=curdate() AND ArrivalLocation Like '%$arrivalcheck%'  ORDER BY DepatureLocation ASC");
            $count3=mysqli_num_rows($query3);
                   
            $query4=mysqli_query($connection,"SELECT * FROM tbl_domestic_flight WHERE Dates like '%$datecheck%' and Dates>=curdate()");
            $count4=mysqli_num_rows($query4);

            $query5=mysqli_query($connection,"SELECT * FROM tbl_domestic_flight WHERE DepatureLocation LIKE '%$depaturecheck%'");
            $count5=mysqli_num_rows($query5);
            
            
            $query6=mysqli_query($connection,"SELECT * FROM  tbl_domestic_flight  WHERE DepatureLocation Like '%$depaturecheck%' AND ArrivalLocation Like '%$arrivalcheck%' and Dates>=curdate() ORDER BY Dates ASC");
                              ...
                              <?php }
                               ...
                               while($row=mysqli_fetch_array($query3))
                                        ...
                                        elseif($count3!==0)
                                        {
                                            $output="<h4><center style='color:red;'>There was no search result!</center></h4>";
                                            print $output ;
                                             $recommendedtickets="<br><h5 style='color:blue'>Recommended Tickets With Same Date and Arrival Location...</h5><br/>";
