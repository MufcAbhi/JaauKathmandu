...
if (isset($_POST['btnSave'])) {
  if ($_FILES['image']['error']==0) {
      ...
      move_uploaded_file($_FILES['image']['tmp_name'], '../img/restaurants/'.$name);
      $blog->set('restaurant_image',$name);
  }
 $blog-> set('restaurant_location',mysql_real_escape_string($_POST['location']));
 //$blog-> set('category',$_POST['category']);
 $blog-> set('restaurant_name',mysql_real_escape_string($_POST['title']));
 //$blog-> set('created_by','admin');
 //$blog-> set('created_date',date('Y-m-d H:i:s'));
 $status=$blog->SaveBlog();
  if ($status !== 0) {
 	$users=$blog->SelectUsers();
 	$blog->set('restaurant_id',$status);
 	foreach ($users as $u) {
 		$blog->set('user_id',$u->ID);
 		$blog->RestaurantRatings();
 	}
 	 echo '
	...
	<div style="margin-top: 25px; height: 80px; margin-bottom: -30px;" align="center" class="alert alert-success alert-dismissable">
            ...
            Restaurant is <strong>successfully </strong> added!
            </div>';
...
<head>
       ...
       <div id="page-wrapper">
            <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header">Add Restaurant</h1>
